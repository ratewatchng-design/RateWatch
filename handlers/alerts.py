...
from database.users_db import is_premium, get_or_create_user
from database.alerts_db import (
    get_user_alerts, create_alert, delete_alert,
    count_user_alerts, FREE_ALERT_LIMIT, FREE_ASSETS
)
...
async def new_alert_target(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if "pct" in condition:
        await query.edit_message_text(
            f"Enter the percentage change to trigger the alert.\n\n"
            f"Example:\n5\n10\n15"
        )
    else:
        await query.edit_message_text(
