...
def main():
    ...
    alert_conv = ConversationHandler(
        ...
        states={
            ...
            ENTERING_TARGET: [MessageHandler(filters.TEXT & ~filters.COMMAND, confirm_remove_alert)],
        },
    ...
    logger.info("RateWatch NG bot starting...")
    app.run_webhook(
        listen="0.0.0.0",
        port=8000,
        webhook_url=f"https://{__import__('os').environ.get('RENDER_EXTERNAL_HOSTNAME', '')}/webhook",
        url_path="/webhook",
    )
